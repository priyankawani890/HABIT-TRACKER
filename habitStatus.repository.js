    ...
    const habitsWithData = await Promise.all(
      habits.map(async (habit) => {
        ...
        const habitStatus = await HabitStatusModel.find({
          habitId: habit._id,
          date: {
            $gte: moment().subtract(7, 'days').startOf('day').toDate(), // Fetch data from the last 7 days
            $lte: moment().endOf('day').toDate(), // Up to today's date
          },
        });
        return { habit, habitStatus }; // Return habit and its status data for the last 7 days
      })
    );
